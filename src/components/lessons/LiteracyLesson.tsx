...
import { CheckCircle, XCircle, ArrowRight, Volume2, Video, X, Star } from "lucide-react";
...
export default function LiteracyLesson({ title, instruction, options, onComplete, videoUrl }: LiteracyLessonProps) {
        ...
        <div className="p-8 text-center">
           {videoUrl && (
             <div className="mb-8">
               <button 
                 ...
                 className="w-full max-w-sm mx-auto bg-amber-500 hover:bg-amber-600 text-white font-bold py-4 px-6 rounded-2xl shadow-lg border-b-4 border-amber-700 active:border-b-0 active:translate-y-1 transition flex items-center justify-center gap-3"
               >
          ...
          <div className="mb-8 relative inline-block">
             <button 
                ...
                className="absolute -right-16 top-1/2 -translate-y-1/2 p-3 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-full hover:scale-110 transition shadow-sm"
                title="Ouvir instrução"
                aria-label="Ouvir instrução"
             >
               <Volume2 className="w-6 h-6" />
             </button>
            <h2 className="text-3xl font-bold text-slate-800 dark:text-slate-100">{instruction}</h2>
          </div>
              ...
              <button
                ...
                className={`
                  ...
                  ${selectedOption === option.id
                    ...
                    : 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-blue-50 dark:hover:bg-blue-900/30 border-2 border-slate-200 dark:border-slate-600'
                  }
                `}
              >
                {option.label}
            ...
            <div className="flex items-center gap-3">
              {isCorrect ? (
                ...
                <div className="flex items-center gap-3">
                   ...
                   <span className="text-xl font-bold text-red-700 dark:text-red-300">Não foi dessa vez. Tente de novo!</span>
                </div>
              )}
            </div>
